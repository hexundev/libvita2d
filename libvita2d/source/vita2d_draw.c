...
{
    if(buffer == NULL || buffer->vertex_type != VITA2D_VERTEX_TYPE_COLOR)
    {
        return;
    }

    vita2d_draw_array(SCE_GXM_PRIMITIVE_TRIANGLES,
                      (vita2d_color_vertex*)buffer->vertices,
                      buffer->vertex_count);
}
void vita2d_draw_vertex_buffer(const vita2d_texture *texture, const vita2d_vertex_buffer *buffer, vita2d_color color)
{
    if(buffer == NULL || texture == NULL || buffer->vertex_type != VITA2D_VERTEX_TYPE_TEXTURE)
    {
        return;
    }

    vita2d_draw_array_textured(texture,
                               SCE_GXM_PRIMITIVE_TRIANGLES,
                               (vita2d_texture_vertex*)buffer->vertices,
                               buffer->vertex_count,
                               color);
}
