...
float _vita2d_model_matrix[4 * 4];
float _vita2d_view_matrix[4 * 4];
float _vita2d_proj_matrix[4 * 4];
...
struct {
    ...
    vita2d_fragment_programs blend_mode_mul;
...
static int vita2d_init_internal(unsigned int temp_pool_size, SceGxmMultisampleMode msaa)
{
    ...
    _vita2d_make_fragment_programs(&_vita2d_fragmentPrograms.blend_mode_mul, &blend_info_mul, msaa);
...
int vita2d_fini()
{
    return vita2d_fini_callback(NULL);
}
int vita2d_fini_callback(vita2d_callback cb)
{
    ...
    _vita2d_free_fragment_programs(&_vita2d_fragmentPrograms.blend_mode_mul);
...
{
    matrix_init_orthographic(_vita2d_ortho_matrix, 0.0f, (float)width, (float)height, 0.0f, 0.0f, 1.0f);
}
...
{
    memcpy(_vita2d_ortho_matrix, mvp, sizeof(_vita2d_ortho_matrix));
}
...
{
    vita2d_vertex_buffer buffer;
    memset(&buffer, 0, sizeof(vita2d_vertex_buffer));

    // get array size
    unsigned int size = 0;
    switch(vertex_type)
    {
        case VITA2D_VERTEX_TYPE_CLEAR:
            size = vertex_count * sizeof(vita2d_clear_vertex);
            break;

        case VITA2D_VERTEX_TYPE_COLOR:
            size = vertex_count * sizeof(vita2d_color_vertex);
            break;

        case VITA2D_VERTEX_TYPE_TEXTURE:
            size = vertex_count * sizeof(vita2d_texture_vertex);
            break;
    ...
    if(buffer.vertices != NULL)
    {
        ...
        buffer.vertex_type = vertex_type;
        buffer.size = size;
    }
...
void vita2d_set_blend_mode_add(int enable)
{
    ...
    vita2d_set_blend_mode(enable ? VITA2D_BLEND_ADD : VITA2D_BLEND_DEFAULT);
}
void vita2d_set_blend_mode(vita2d_blend_mode mode)
{
    vita2d_fragment_programs *in = NULL;
    switch(mode)
    {
        case VITA2D_BLEND_DEFAULT:
            in = &_vita2d_fragmentPrograms.blend_mode_normal;
            break;

        case VITA2D_BLEND_ADD:
            in = &_vita2d_fragmentPrograms.blend_mode_add;
            break;

        case VITA2D_BLEND_MULTIPLY:
            in = &_vita2d_fragmentPrograms.blend_mode_mul;
            break;
